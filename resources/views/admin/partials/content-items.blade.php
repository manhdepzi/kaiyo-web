        ...
        @if($adminNavigation['content'] ?? false)
            <details class="mt-3"><summary class="cursor-pointer font-semibold text-brand">Tạo revision thay thế</summary>
                <form method="POST" action="{{ route($reviseRoute, $item['public_id']) }}" class="mt-3 grid gap-2">@csrf<input type="hidden" name="lock_version" value="{{ $item['lock_version'] }}">
                    ...
                    @if($type === 'banners')
                        <x-ui.input name="headline" label="Tên mô tả slide mới" :value="$item['label']" required />
                        <label class="grid gap-1 text-sm font-medium">Ảnh thiết kế
                            <select name="image_path" required class="rounded-control border border-line bg-surface px-3 py-2">
                                @foreach($slideImages as $path => $label)<option value="{{ $path }}" @selected($item['image_path'] === $path)>{{ $label }}</option>@endforeach
                            </select>
                        </label>
                        <x-ui.input name="sort_order" type="number" label="Thứ tự" :value="$item['sort_order']" required />
                        <x-ui.input name="body" label="Nội dung dự phòng" />
                        <x-ui.input name="cta_label" label="CTA label" value="Liên hệ ngay" />
                        <x-ui.input name="cta_url" label="CTA URL" value="/lien-he" />
                    @endif
                    ...
                    <x-ui.button type="submit" size="sm" icon="check">Lưu revision</x-ui.button>
                </form>
            </details>
            @if($type !== 'email-templates')
                <details class="mt-3"><summary class="cursor-pointer font-semibold text-brand">Media của revision</summary>
                    <div class="mt-2 space-y-2">@foreach($item['media'] as $reference)<div class="flex items-center justify-between gap-2"><span>{{ $reference['original_name'] }} · {{ $reference['purpose'] }}</span><form method="POST" action="{{ route('admin.content.media.detach', ['type'=>$type,'content'=>$item['public_id'],'asset'=>$reference['public_id'],'purpose'=>$reference['purpose']]) }}">@csrf @method('DELETE')<input type="hidden" name="lock_version" value="{{ $item['lock_version'] }}"><x-ui.button type="submit" size="sm" variant="secondary" icon="x-mark">Gỡ</x-ui.button></form></div>@endforeach</div>
                    <form method="POST" action="{{ route('admin.content.media.attach', ['type'=>$type,'content'=>$item['public_id']]) }}" class="mt-3 grid gap-2">@csrf<input type="hidden" name="lock_version" value="{{ $item['lock_version'] }}"><x-ui.input name="asset_public_id" label="Media public ID" required /><x-ui.input name="purpose" label="Purpose" required /><x-ui.input name="sort_order" type="number" label="Thứ tự" value="0" required /><x-ui.button type="submit" size="sm" icon="paper-clip">Gắn media</x-ui.button></form>
                </details>
            @endif
        @endif
        @if($adminNavigation['content_publish'] ?? false)<div class="mt-3 flex gap-2">
            @if(!$item['has_published_revision'])<form method="POST" action="{{ route($publishRoute,$item['public_id']) }}">@csrf<input type="hidden" name="lock_version" value="{{ $item['lock_version'] }}"><x-ui.button type="submit" size="sm" icon="cloud-arrow-up">Xuất bản</x-ui.button></form>@endif
            @if($item['has_published_revision'])<form method="POST" action="{{ route('admin.content.unpublish', ['type'=>$type,'content'=>$item['public_id']]) }}">@csrf<input type="hidden" name="lock_version" value="{{ $item['lock_version'] }}"><x-ui.button type="submit" size="sm" variant="secondary" icon="cloud-arrow-down">Gỡ xuất bản</x-ui.button></form>@endif
        </div>@endif
