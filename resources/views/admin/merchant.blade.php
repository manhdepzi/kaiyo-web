    ...
    <div class="mt-8 grid gap-8 lg:grid-cols-[22rem_1fr]">
        <x-ui.card title="Tạo batch">
            <form method="POST" action="{{ route('admin.merchant.store') }}" class="space-y-4">
                ...
                <x-ui.button type="submit">Đưa vào hàng đợi</x-ui.button>
            </form>
        </x-ui.card>
        <section>
            ...
            <div class="space-y-3">
                @forelse($batches as $batch)
                    <x-ui.card :title="$batch->configuration_revision.' · '.$batch->state">
                        ...
                        @if(in_array($batch->state,['partial','failed'],true))<form class="mt-3" method="POST" action="{{ route('admin.merchant.retry',$batch->public_id) }}">@csrf<x-ui.button type="submit" variant="secondary" size="sm">Retry lỗi</x-ui.button></form>@endif
                    </x-ui.card>
                @empty
