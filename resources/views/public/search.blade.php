...
<section class="mx-auto max-w-7xl px-5 py-12 lg:px-8">
    <div class="max-w-3xl">
        ...
        <form action="{{ route('public.search') }}" method="GET" role="search" class="mt-7 flex flex-col gap-3 sm:flex-row">
            ...
            <x-ui.button type="submit" class="self-end">Tìm kiếm</x-ui.button>
        </form>
    </div>
    ...
    @if (count($result->hits) === 0)
        <x-ui.empty-state class="mt-10" title="Không tìm thấy sản phẩm" description="Hãy kiểm tra chính tả, thử SKU khác hoặc dùng từ khóa ngắn hơn.">
            @if ($term !== '')<x-ui.button :href="route('public.search')" variant="secondary">Xóa tìm kiếm</x-ui.button>@endif
        </x-ui.empty-state>
    @else
        ...
        <nav class="mt-8 flex items-center justify-between" aria-label="Phân trang kết quả">
            @if ($result->page > 1)
                <x-ui.button :href="route('public.search', ['q' => $term, 'page' => $result->page - 1])" variant="secondary">Trang trước</x-ui.button>
            @else
                <span></span>
            @endif
            @if ($result->hasMore)
                <x-ui.button :href="route('public.search', ['q' => $term, 'page' => $result->page + 1])" variant="secondary">Trang sau</x-ui.button>
            @endif
        </nav>
    @endif
</section>
