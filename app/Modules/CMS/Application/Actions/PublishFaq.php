...
final readonly class PublishFaq
{
    public function __construct(private PermissionAuthorizer $authorizer) {}

    public function execute(UserAccount $actor, Faq $faq, int $expectedVersion): Faq
    {
        ...
        return DB::transaction(function () use ($faq, $expectedVersion): Faq {
            ...
            if ($locked->lock_version !== $expectedVersion || $locked->current_revision_id === null) {
                throw new DomainException('FAQ changed before publication.');
            }
            $revision = FaqRevision::query()->whereKey($locked->current_revision_id)->where('faq_id', $locked->getKey())->firstOrFail();
