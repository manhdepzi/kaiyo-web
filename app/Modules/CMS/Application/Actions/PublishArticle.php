...
final readonly class PublishArticle
{
    ...
    public function execute(UserAccount $actor, Article $article, int $expectedVersion): Article
    {
        ...
        return DB::transaction(function () use ($article, $expectedVersion): Article {
            ...
            if ($locked->lock_version !== $expectedVersion || $locked->current_revision_id === null) {
                throw new DomainException('Article changed before publication.');
            }
            $revision = ArticleRevision::query()->whereKey($locked->current_revision_id)->where('article_id', $locked->getKey())->firstOrFail();
