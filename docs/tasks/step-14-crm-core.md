...
- [x] Normal-impact capability delegation requires the actor to hold the same permission persistently at the target Company scope; high-impact capability requests fail closed into the governed dual-control grant workflow.
- [x] Membership creation, successful Company capability grants and effective revocations append authorization audit evidence without storing raw permission snapshots; repeated revocation is idempotent and does not duplicate audit events.
...
- Current CRM slice: 9 tests / 39 assertions; current full SQLite regression: 209 tests / 1,707 assertions with four documented MySQL-trigger skips.
- Original Step 14 closure regression: 34 tests / 325 assertions; CRM slice: 8 tests / 30 assertions.
