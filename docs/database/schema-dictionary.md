...
| `products` | `brand_id NULL`, `primary_category_id`, `name`, `slug`, `status`, `description`, `lock_version`, `deleted_at NULL` | unique reserved slug; status `draft/active/inactive` | deactivate; issued snapshots unaffected |
...
| `catalog_media_references` | `product_id NULL`, `variant_id NULL`, `media_asset_id`, `purpose`, `sort_order` | exactly one catalog owner; unique owner+asset+purpose | explicit detach; media ownership retained |
...
| `notifications` | public ID, channel, recipient reference/destination encrypted or tokenized, template revision, business fact reference, state, idempotency key | unique idempotency key; state queued/sending/sent/failed/dead | operational/privacy retention |
| `notification_attempts` | notification ID, attempt no, provider code/ref hash, outcome/error/latency, attempted at | unique notification+attempt; no secret/full sensitive payload | operational retention |
