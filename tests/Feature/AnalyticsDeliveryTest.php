...
final class AnalyticsDeliveryTest extends TestCase
{
    ...
    public function test_consent_is_fail_closed_and_destination_event_identity_is_deduplicated(): void
    {
        ...
        $batch = app(DeliverAnalyticsBatch::class)->execute('ga4', 'analytics-v1', 'consent-v1', 'analytics-operation-001', $events);
        ...
        $retry = app(DeliverAnalyticsBatch::class)->execute('ga4', 'analytics-v1', 'consent-v1', 'analytics-operation-001', $events);
        self::assertSame($batch->getKey(), $retry->getKey());
        self::assertCount(1, $destination->calls);

        $duplicateBatch = app(DeliverAnalyticsBatch::class)->execute('ga4', 'analytics-v1', 'consent-v1', 'analytics-operation-002', $events);
    ...
    public function test_provider_failure_is_visible_and_retry_uses_the_same_item(): void
    {
        ...
        $failed = app(DeliverAnalyticsBatch::class)->execute('ga4', 'analytics-v1', 'consent-v1', 'analytics-operation-003', $events);
        ...
        $completed = app(DeliverAnalyticsBatch::class)->execute('ga4', 'analytics-v1', 'consent-v1', 'analytics-operation-003', $events);
    ...
    public function test_event_catalog_rejects_unknown_types_and_raw_personal_data(): void
    {
        foreach ([
            ...
            try {
                app(DeliverAnalyticsBatch::class)->execute('ga4', 'analytics-v1', 'consent-v1', 'analytics-invalid-'.$index, [$event]);
    ...
    {
        return new AnalyticsEvent($identity, $type, 'commerce', '01KAIYOANALYTICSSUBJECT01', new DateTimeImmutable('2026-08-26T09:00:00+07:00'), $consent, $attributes);
    }
